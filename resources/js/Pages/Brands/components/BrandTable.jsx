...
import { accentStyles } from '../utils/brandHelpers';

export function BrandTable({ datasList = [], onEdit, onDelete }) {
    return (
                                ...
                                <tr key={item.id} className="transition hover:bg-emerald-50/30">
                                    ...
                                    <td className="px-5 py-4">
                                        <div className="flex justify-end gap-2">
                                            ...
                                            <button onClick={() => onDelete(item)} type="button" className="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-rose-100 bg-rose-50 text-rose-600 transition hover:bg-rose-100 cursor-pointer" aria-label={`Delete ${item?.brand_title || 'brand'}`} title="Delete">
