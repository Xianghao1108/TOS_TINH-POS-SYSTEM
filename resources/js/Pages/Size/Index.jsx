...
export default function SizesPage({ auth, sizes, filters }) {
    ...
    const renderSizeForm = ({ mode, values, setValues, formErrors, busy, onCancel }) => {
        ...
        return (
            <form onSubmit={isEdit ? submitEdit : submitAdd} noValidate className="p-6">
                <div className="mb-6 flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-lg font-bold text-slate-950">{isEdit ? 'Edit Size' : 'Add New Size'}</h2>
                ...
                <div>
                    <label htmlFor={isEdit ? 'edit_size_title' : 'size_title'} className="block text-sm font-semibold text-slate-800">
                        Size title <span className="text-rose-500">*</span>
                    </label>
                    <input
                        id={isEdit ? 'edit_size_title' : 'size_title'}
                        type="text"
                        className={inputClass(sizeExistsError || formErrors.size_title)}
                        value={values.size_title}
                        onChange={(e) => setValues('size_title', e.target.value)}
                        onBlur={(e) => handleCheckExists(e.target.value, isEdit)}
                        placeholder="M"
                    />
                    <InputError message={sizeExistsError || formErrors.size_title} className="mt-2" />
                </div>
                ...
                <div className="min-h-[calc(100vh-140px)] bg-[#F2F9F5] px-4 py-6 sm:px-6 lg:px-8">
                    <div className="mx-auto max-w-7xl">
                        <div className="mb-8 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
                            ...
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                                <form onSubmit={handleSearch} className="relative w-full sm:w-72">
                                    ...
                                    />
                                    <button type="submit" className="absolute left-3 top-1/2 flex h-5 w-5 -translate-y-1/2 items-center justify-center text-slate-400 transition hover:text-emerald-600" aria-label="Search sizes">
                                                    ...
                                                    <tr key={item.id} className="transition hover:bg-emerald-50/30">
                                                        <td className="px-5 py-4">
                                                            <div className="flex items-center gap-3">
                                                                ...
                                                                <div>
                                                                    <p className="text-base font-bold text-slate-950">{item?.size_title}</p>
                                                                    <p className="mt-0.5 text-xs font-semibold text-slate-400">Size #{item?.id}</p>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td className="px-5 py-4">
                                                        ...
                                                        <td className="px-5 py-4">
                                                            <div className="flex justify-end gap-2">
                                                                ...
                                                                <button onClick={() => confirmDataDeletion(item)} type="button" className="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-rose-100 bg-rose-50 text-rose-600 transition hover:bg-rose-100" aria-label={`Delete ${item?.size_title || 'size'}`} title="Delete">
                        ...
                        <Modal show={isModalOpen} onClose={closeAddModal}>
                            {renderSizeForm({ mode: 'add', values: data, setValues: setData, formErrors: errors, busy: processing, onCancel: closeAddModal })}
                        </Modal>

                        <Modal show={isEditModalOpen} onClose={closeEditModal}>
                            {renderSizeForm({ mode: 'edit', values: editData, setValues: setEditData, formErrors: editErrors, busy: editProcessing, onCancel: closeEditModal })}
                        </Modal>
                        ...
                        <Modal show={confirmingDataDeletion} onClose={closeModal}>
                            <form onSubmit={deleteDataRow} className="p-6">
                                ...
                                <h2 className="text-lg font-bold text-slate-950">Delete size?</h2>
                                <p className="mt-2 text-sm text-slate-600">
                                    Are you sure you want to delete <span className="font-bold text-slate-900">{dataEdit?.size_title}</span>?
                                </p>
