...
class KhqrService
{
    ...
    public function getAccessToken(): ?string
    {
        ...
        if (! $apiUrl || ! $apiEmail) {
            Log::warning('Bakong renew_token skipped: missing api_url or api_email config');
        ...
        return Cache::remember('bakong_access_token', 1800, function () use ($apiUrl, $apiEmail) {
            try {
                $response = Http::withoutVerifying()->timeout(5)->post($apiUrl.'v1/renew_token', [
                    'email' => $apiEmail,
                ]);
                ...
                if ($response->successful()) {
                    ...
                    Log::error('Bakong renew_token returned an application-level error', [
                ...
                Log::error('Bakong renew_token failed', [
                    'status' => $response->status(),
                    'body' => $response->body(),
                ]);
            } catch (\Exception $e) {
                Log::error('Bakong renew_token exception', ['message' => $e->getMessage()]);
            }
    ...
    public function checkTransaction(string $md5Hash): ?array
    {
        ...
        if (! $apiUrl || ! $apiToken) {
            Log::warning('Bakong verification skipped: No API url or access token available');
        ...
        while ($attempts < 2) {
            try {
                ...
                if ($response->status() === 401 && $attempts === 0) {
                    Log::warning('Bakong token returned 401 Unauthorized. Fetching a fresh dynamic token...');
                    Cache::forget('bakong_access_token');
                    $apiToken = $this->getAccessToken();
                    if (! $apiToken) {
                        Log::warning('Bakong token refresh failed after 401 Unauthorized');
                ...
                Log::warning('Bakong check_transaction_by_md5 response unsuccessful', [
                    'status' => $response->status(),
                    'body' => $response->body(),
                ]);
            } catch (\Exception $e) {
